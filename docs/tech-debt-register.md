...
| TD-008 | Security | CSP — shipped **Report-Only** (`next.config.ts`); enforce after a browser-console pass confirms nothing legit is blocked | `next.config.ts` | S (remaining) | Med | — | 2026-05-22 | Next | enforced CSP needs browser verification (next-themes inline + konva/tfjs) |
