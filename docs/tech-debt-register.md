...
| TD-002 | Code Quality | 17 type escapes (`as any`/`as unknown as`/`@ts-expect-error`) | `supabase/queries.ts`, `LanguageSwitch`, `billing/server`, `storage/supabase*`, `stripe/webhook`, +4 | M | Med | 2.0 | 2026-05-22 | Next | RPC + 3rd-party (FormData/JWT) typing gaps |
